...
export default function AddOffer({ displayType = false }) {
    ...
    function handleAddNewOffer(e) {
        if(e === 'confirmed') {
            fetchAddOffer({ description: description, name: title, price: price, quantity: maxQuantity.current.value, type: type, url: imgUrl });
        } else {
            e.preventDefault();
        ...
        <>
            ...
            { previewOffer && <Modal unmount={() => setPreviewOffer(false)}>
                <form>
                    ...
                    <button className="add-offer-confirm" onClick={() => handleAddNewOffer('confirmed')}>Confirm</button>
                </form>
            </Modal> }
            ...
            <form className="offer add-offer" onSubmit={handleAddNewOffer}>
                ...
                { displayType && <p className="offer-type" contentEditable={true} suppressContentEditableWarning={true} onInput={(e) => setType(e.target.innerText)}>Set Type</p> }
                <button className="admin-plus-button" style={{border: 'solid black 1px'}}><span>+</span></button>
